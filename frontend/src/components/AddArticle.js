...
const AddArticle = () => {
    ...
    var [myvalue, setmyValue] = MyForm({ username: "", title: "", content: "",course:""})
    const addArticles = () => {
        console.log(myvalue)
        axios.post("http://localhost:3005/api/add", myvalue).then(
            (res) => {
                alert("Successfully Added Associate")
    ...
    <div className="form">
      ...
      <div className="form-group mt-3">
        ...
        <div className="form-group mt-3">
          ...
          <input type="text" className="form-control" name="content" id="username" placeholder="Enter Associate Name" value={myvalue.content} onChange={setmyValue}/>
        </div>
        ...
        <div className="btn-lrg submit-btn" onClick={addArticles}>Submit</div>
      </form>
    </div>
  </div>
</section>
